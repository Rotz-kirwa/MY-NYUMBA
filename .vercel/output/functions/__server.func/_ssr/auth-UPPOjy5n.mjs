...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-UPPOjy5n.js
...
async function getClientInstance() {
	...
	try {
		const { createClient } = await import("../_libs/@libsql/client.mjs").then((n) => n.n);
		if (dbUrl.startsWith("http:") || dbUrl.startsWith("https:") || dbUrl.startsWith("libsql:") || dbUrl.startsWith("ws:") || dbUrl.startsWith("wss:")) clientInstance = createClient({ url: dbUrl });
		else clientInstance = dummyClient;
	} catch (err) {
